...
This agent depends on the utility library [graph-utils](../graphUtils/src/searchClient.ts) to implement different search actions.

The email agent uses the Microsoft Graph API to interact with the user's SharePoint and OneDrive files. The agent uses `@microsoft/microsoft-graph-client` library to interact with the Microsoft Graph API. The agent enables operations to find files.

To build the search agent, it needs to provide a manifest and an instantiation entry point.  
...
When loading search agent in a NPM package, the dispatcher first loads the [searchManifest.json](./src/searchManifest.json).
...
```
Find files detailing parental leave policies
```
